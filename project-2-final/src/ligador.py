...
def finalizar_ligacao():
    with (open("data/interim/ingredientes.csv", 'r') as ing_f,
        ...
        header = ["id_cdb", "nome_cdb", "id_fdb",
                  "nome_fdb", "grupo", "subgrupo", "aliases"]
        ...
        for ing in ing_m_reader:
            if ing["id_fdb"] != "":
                int(ing["id_fdb"])
                ing_writer.writerow(
                    {**ing, **{"aliases": ingredientes[ing["id_cdb"]]["Ingredient Synonyms"]}})
            else:
                ing_r_writer.writerow(
                    {**ing, **{"aliases": ingredientes[ing["id_cdb"]]["Ingredient Synonyms"]}})
        ...
        for ing in ingc_m_reader:
            if ing["id_fdb"] != "":
                int(ing["id_fdb"])
                ing_writer.writerow(
                    {**ing, **{"aliases": ingredientes_compostos[ing["id_cdb"]]["Compound Ingredient Synonyms"]}})
            else:
                ingc_writer.writerow(
                    {**ing, **{"aliases": ingredientes_compostos[ing["id_cdb"]]["Compound Ingredient Synonyms"]}})
...
def ligar_ingrediente_receita():
    with (open("data/interim/ingredientes_final.csv") as ing_f,
        ...
        map_final: dict[(str, str), dict] = dict()
        for rec_ing in rec_ing_reader:
            ...
            if ing := ingredientes.get(rec_ing["Entity ID"], False):
                ...
                if tem_volume:
                    if (id_ing, id_receita) in map_final.keys():
                        map_final[(id_ing, id_receita)]["volume"] += calc_volume(q, u)
                    else:
                        map_final[(id_ing, id_receita)] = {
                    ...
                    if (id_ing, id_receita) in map_final.keys():
                        map_final[(id_ing, id_receita)]["massa"] += converte_pound(q)
                    else:
                        map_final[(id_ing, id_receita)] = {
                            "id_ingrediente": ing["id_fdb"],
                            "id_receita": rec_ing["Recipe ID"],
                    ...
                    if (id_ing, id_receita) in map_final.keys():
                        map_final[(id_ing, id_receita)]["unidade"] += int(q)
                    else:
                        map_final[(id_ing, id_receita)] = {
                            "id_ingrediente": ing["id_fdb"],
                            "id_receita": rec_ing["Recipe ID"],
        ...
        out_writer.writerows(map_final[x] for x in map_final)
